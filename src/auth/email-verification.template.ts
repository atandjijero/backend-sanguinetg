...
export function genererEmailVerification(data: EmailVerificationData): string {
  return `<!DOCTYPE html>
<html lang="fr">
          ...
          <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.08);">
            ...
            <tr>
              <td style="padding:32px;">
                <p style="margin:0 0 4px;font-size:13px;color:#b3131c;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;">Vérification de votre email</p>
                <h1 style="margin:0 0 16px;font-size:22px;color:#1a1a1a;">Bienvenue ${data.prenom}</h1>
                <p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:#3f3f3f;">
                  Merci de rejoindre le réseau de donneurs Sanguine TG. Confirmez votre adresse email en cliquant sur
                  le bouton ci-dessous pour activer votre compte. Ce lien expire dans 24 heures.
                </p>
                <table role="presentation" cellpadding="0" cellspacing="0" style="margin:8px 0 24px;">
                  <tr>
                    <td>
                      <a href="${data.lienVerification}" style="display:inline-block;background-color:#b3131c;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:12px 22px;border-radius:8px;">Vérifier mon email</a>
                    </td>
                  </tr>
                </table>
